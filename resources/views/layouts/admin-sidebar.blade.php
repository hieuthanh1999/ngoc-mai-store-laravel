<body class="hold-transition sidebar-mini layout-fixed">
    <div class="wrapper" style="background: rgb(28, 47, 127)">
      ...
      <aside class="main-sidebar sidebar-dark-primary elevation-4">
        <!-- Brand Logo -->
        <a href="{{ route('admin.home') }}" class="brand-link"  style="margin-left:  10px">
          ...
          <span class="brand-text font-weight-light">
            ...
            {{-- {{ (Auth::guard('admin')->user()->role_id == 1) ? 'Hệ thống quản trị' : 'Hệ thống nhân viên'}} --}}
          </span>
        </a>

        <!-- Sidebar -->
        <div class="sidebar" style="background: rgb(28, 47, 127)">
          <!-- Sidebar user panel (optional) -->
          <div class="user-panel mt-3 pb-3 mb-3 d-flex">
            <div class="image">
              <img src="{{ asset('asset/admin/dist/img/user.png') }}" class="img-circle elevation-2" alt="User Image">
            </div>
            <div class="info">

              <a href="javascrip:void(0)" class="d-block">Xin chào, {{ Auth::guard('admin')->user()->name }}</a>
            </div>
          </div>
          ...
          <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
              ...
              <li class="nav-item">
                @php
                    $isRouteUser = request()->is('admin/banners*');
                @endphp
                <a href="{{ route('admin.banners_index') }}" class="nav-link {{ ($isRouteUser) ? 'active' : '' }}">
                    <i class="nav-icon fas fa-flag"></i> <!-- Icon -->
                    <p>
                    {{ TextLayoutSidebar("banner") }}
                  </p>
                </a>
              </li>
              <li class="nav-item">
                @php
                    $isRouteUser = request()->is('admin/news*');
                @endphp
                <a href="{{ route('admin.news_index') }}" class="nav-link {{ ($isRouteUser) ? 'active' : '' }}">
                    <i class="nav-icon fas fa-newspaper"></i><!-- Icon -->
                    <p>
                    {{ TextLayoutSidebar("news") }}
                  </p>
                </a>
              </li>
              {{-- nếu người dùng đang đăng nhập là quản trị viên thì hiển thị chức năng này --}}
