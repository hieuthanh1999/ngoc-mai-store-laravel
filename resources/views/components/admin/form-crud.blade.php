...
<form action="{{ $route }}" method="POST" id="form__js" enctype="multipart/form-data">
  ...
  <!-- /.card-body -->
  <div class="card-header d-flex justify-content-between align-items-center" style="padding-bottom: 10px;">
    <!-- Nút Hủy nằm bên trái (nếu cần thiết) -->
    @if ($cancelBtn == "true")
        <a href="{{ route($cancel) }}" class="btn btn-danger next-link__js">Hủy</a>
    @endif

    <!-- Nút Thêm Mới nằm bên phải -->
    <button class="btn btn-success ml-auto">{{$textSubmit}}</button>
</div>
